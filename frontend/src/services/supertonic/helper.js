...
export async function loadCfgs(onnxDir) {
    ...
    if (!response.ok) {
        throw new Error(`Failed to load tts.json (${response.status})`);
    }
...
export async function loadTextProcessor(onnxDir) {
    ...
    if (!response.ok) {
        throw new Error(`Failed to load unicode_indexer.json (${response.status})`);
    }
 ...
 */
export async function loadTextToSpeech(onnxDir, sessionOptions = {}, progressCallback = null, configDir = onnxDir) {
    console.log('Using WebAssembly/WebGPU for inference');
    
    const cfgs = await loadCfgs(configDir);
    ...
    const textProcessor = await loadTextProcessor(configDir);
